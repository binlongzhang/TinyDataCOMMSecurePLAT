...
int TcpSocket::connectToHost(char * ip, unsigned short port, int timeout)
{
	...
	{
		ret = ParamError;
		m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func sckClient_connect() err, check  (ip==NULL || connfd==NULL || port<=0 || port>65537 || connecttime < 0)");
		return ret;
	}
	...
	if (m_socket < 0){
		ret = errno;
		m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func socket() err");
		return ret;
	}
	...
	if (ret < 0)
	{
		...
		else
		{
			...
			m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func connect_timeout() err");
		}
	}
...
/*
* sendMsg - 发送message,成功返回0，失败 返回发送的字符或者-1，完整message应该为 dataLen + sizeof(int)
* @sendData: message
* @dataLen:	message长度，单位是字节
* @timeout: 超时时间
*/
int TcpSocket::sendMsg(char * sendData, int dataLen, int timeout)
{
	...
	{
		ret = ParamError;
		m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func sendMsg() err, check (data == NULL || datalen <= 0)");
		return ret;
	}
	...
	{
		int writed = 0;
		unsigned char *netdata = (unsigned char *)malloc(dataLen + sizeof(int));
		if (netdata == NULL)
		{
			ret = MallocError;
			m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func sendMsg() malloc err,");
			printf("func sendMsg() malloc Err:%d\n ", ret);
			return ret;
		}
		int netlen = htonl(dataLen);
		memcpy(netdata, &netlen, sizeof(int));
		memcpy(netdata + sizeof(int), sendData, dataLen);

		writed = writen(netdata, dataLen + sizeof(int));
		if (netdata != NULL){
			free(netdata);
			netdata = NULL;
		}
	...
	if (ret < 0)
	{
		...
		{
			ret = TimeoutError;
			m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func sendMsg() write time out");
...
/*
* recvMsg - 接受message,不需要提前申请内存，但要注意记得使用freeMemory释放释放
* @recvData: 输出数据地址
* @dataLen:	message长度，单位是字节
* @timeout: 超时时间
*/
int TcpSocket::recvMsg(char ** recvData, int & recvLen, int timeout)
{
	...
	if (recvData == NULL){
		ret = ParamError;
		m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func recvMsg() err");
	...
	if (ret != 0)
	{
		...
		{
			ret = TimeoutError;
			m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func read_timeout() timeout");
			return ret;
		}
		else
		{
			m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func read_timeout() err");
			return ret;
		}
	}
	...
	if (ret == -1){
		m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func readn() err");
		return ret;
	}else if (ret < 4){
		ret = PeerCloseError;
		//printf("func readn() err peer closed:%d \n", ret);
		m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func readn() err, peer closed");
		return ret;
	}
	...
	{
		ret = MallocError;
		m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "malloc() err");
		return ret;
	}
	...
	if (ret == -1)
	{
		...
		m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "readn() err");
		return ret;
	}else if (ret < n){
		ret = PeerCloseError;
		//printf("func readn() err peer closed:%d \n", ret);
...
int TcpSocket::blockIO(int fd)
{
	...
	{
		ret = flags;
		m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func activate_nonblock() err");
		return ret;
	}
...
int TcpSocket::noBlockIO(int fd)
{
	...
	{
		ret = flags;
		m_log.Log(__FILE__, __LINE__, MyLog::ERROR, ret, "func deactivate_nonblock() err");
		return ret;
	}
...
/*
* writeTimeout - 写超时检测函数，不含写操作,时间<0则不写
* @wait_seconds: 等待超时秒数，如果为0表示不检测超时
* 成功（未超时）返回0，失败返回-1，超时返回-1并且errno = ETIMEDOUT
*/
...
/*
* readn - 读取固定字节数，循环从buf中读，返回读取到的字符长度
* @fd: 文件描述符
* @buf: 接收缓冲区
* @count: 要读取的字节数
...
/*
* writen - 发送固定字节数，循环写如buf，返回写出去的字符长度
* @buf: 发送缓冲区
* @count: 要读取的字节数
* 成功返回count，失败返回-1
*/
